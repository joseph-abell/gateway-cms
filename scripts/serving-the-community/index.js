...
let {
  data: { pages: { "-K1tQ0jUydotRM_j2uLI": oldData } }
} = require("../old.json");
    ...
    if (isFullScreen) {
      return {
        result: [...acc.result, {
          content: {
            left: {
              deck: item.content && stripHtml(item.content) || item.image_title,
              image: imageTest,
...
fs.writeFileSync('./data/pages/serving-the-community.json', JSON.stringify(newData, null, 2), 'utf8');